...
# Should be located on SSAAT/src directory
...
def run(args, label_mask_path=None):
    audio_conf = {'num_mel_bins': args.num_mel_bins, 'target_length': args.target_length, 'freqm': args.freqm,
    ...
    if 'pretrain' not in args.task:
        ...
        if label_mask_path is None:
            stats_list = train(audio_model, train_loader, val_loader, args)
        else:
            with open(label_mask_path, 'r') as f:
                label_mask = json.load(f)['label_mask']
                label_mask = torch.tensor(label_mask)
            print(f'Using label mask:\n {label_mask}')
            stats_list = train(audio_model, train_loader, val_loader, args, label_mask=label_mask)
    else:
    ...
    if args.data_eval != None:
        ...
        # Ron: should be cross entropy:
        # args.loss_fn = torch.nn.BCEWithLogitsLoss()
        stats, _ = validate(audio_model, val_loader, args, 'valid_set', label_mask=label_mask.to(device), mode='valid')
        # note it is NOT mean of class-wise accuracy
        val_acc = stats[0]['acc']
        val_birds_acc = stats[0]['birds_acc']
        val_mAUC = np.mean([stat['auc'] for stat in stats])
        print('---------------evaluate on the validation set---------------')
        print("Accuracy: {:.6f}".format(val_acc))
        print("Birds Accuracy: {:.6f}".format(val_birds_acc))
        ...
        stats, _ = validate(audio_model, eval_loader, args, 'eval_set', label_mask=label_mask, mode='eval')
        eval_acc = stats[0]['acc']
        eval_birds_acc = stats[0]['birds_acc']
        eval_mAUC = np.mean([stat['auc'] for stat in stats])
        print('---------------evaluate on the test set---------------')
        print("Accuracy: {:.6f}".format(eval_acc))
        print("Birds Accuracy: {:.6f}".format(eval_birds_acc))
...
def eval_model(audio_model, model_params_path, args, no_target_stats=False):
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # load model
    sd = torch.load(model_params_path, map_location=device)
    if not isinstance(audio_model, torch.nn.DataParallel):
        audio_model = torch.nn.DataParallel(audio_model)
    audio_model.load_state_dict(sd, strict=False)
