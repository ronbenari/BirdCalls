...
class ASTModel(nn.Module):
    def __init__(self, label_dim=527,
        ...
        # use a pretrained models for finetuning
            ...
            # pavel try 2 layers for fine tuning
            # self.mlp_head = nn.Sequential(nn.LayerNorm(self.original_embedding_dim),
	    ...
	    # pavel try 3 layers for fine tuning
            # self.mlp_head = nn.Sequential(nn.LayerNorm(self.original_embedding_dim),
            #                                nn.Linear(self.original_embedding_dim, self.original_embedding_dim),
            #                                nn.ReLU(),
            #                                nn.LayerNorm(self.original_embedding_dim),
	    # 			   nn.Linear(self.original_embedding_dim, self.original_embedding_dim),
            ...
            self.v.patch_embed.num_patches = num_patches
            print(f'f_dim={f_dim}, t_dim={t_dim}')
            print(f'p_f_dim={p_f_dim}, p_t_dim={p_t_dim}')
            print(f'fstride={fstride}, tstride={tstride}, input_fdim={input_fdim}, input_tdim={input_tdim}' +
                        f', fshape={fshape}, tshape={tshape}')
