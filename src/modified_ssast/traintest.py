...
def train(audio_model, train_loader, test_loader, args, label_mask=None):
    ...
    # Birds calls with BCE loss
    print('scheduler for bird calls is used')
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, list(range(5, 26)), gamma=0.85)
    main_metrics = 'acc'
    # loss_fn = nn.CrossEntropyLoss(label_smoothing=0.1)

    # Using BCE loss for binary loss per label
    # When masking labels we need access to the loss of each parameter
    pos_weight = torch.tensor(args.pos_weight).to(device)
    loss_reduction = 'mean'
    if label_mask is not None:
        loss_reduction = 'none'
        label_mask = label_mask.to(device)
    loss_fn = nn.BCEWithLogitsLoss(reduction=loss_reduction, pos_weight=pos_weight)
    warmup = False
    ...
    while epoch < args.n_epochs + 1:
        ...
        for i, (audio_input, labels) in tqdm(enumerate(train_loader))   :
            ...
            if isinstance(loss_fn, torch.nn.CrossEntropyLoss):
                ...
                # Mask labels if label_mask is not None, do mean reduction on filtered values
                if label_mask is not None:
                    label_mask = label_mask.to(loss.device)
                    loss = (loss*label_mask).sum() / label_mask.sum() / loss.shape[0]
        ...
        # Ron: Disable validation for smooth run
        print('start validation')
        # # print('*** running validate function')
        stats, valid_loss = validate(audio_model, test_loader, args, epoch, label_mask=label_mask, mode='test')
        stats_list.append(stats)

        #
        # # ensemble results
        # print('*** running validate_ensemble function')
        ...
        acc = stats[0]['acc']
        birds_acc = stats[0]['birds_acc']
        birds_precision = stats[0]['birds_precision']
        birds_recall = stats[0]['birds_recall']
        #
        middle_ps = [stat['precisions'][int(len(stat['precisions'])/2)] for stat in stats]
        middle_rs = [stat['recalls'][int(len(stat['recalls'])/2)] for stat in stats]
        average_precision = np.mean(middle_ps)
        average_recall = np.mean(middle_rs)
        # # Dummy precision and recall
        # average_precision = 0
        # average_recall = 0
        #
        if main_metrics == 'mAP':
            ...
            print("birds_acc: {:.6f}".format(birds_acc))
            print("birds_precision: {:.6f}".format(birds_precision))
            print("birds_recall: {:.6f}".format(birds_recall))
    ...
    now_str = datetime.datetime.now().strftime("%Y_%m_%d-%H_%M")
    json_save_path = exp_dir + '/stats_save ' + now_str + '.json'
...
def validate(audio_model, val_loader, args, epoch, label_mask=None, mode=''):
    ...
    with torch.no_grad():
        for i, (audio_input, labels) in enumerate(val_loader):
            ...
            # compute output
            # print('*** @validate run model')
            audio_output = audio_model(audio_input, args.task)
            # print('*** @validate sigmoid')
            ...
            # compute the loss
            # print('*** @validate compute loss')
            labels = labels.to(device)
            if isinstance(args.loss_fn, torch.nn.CrossEntropyLoss):
                loss = args.loss_fn(audio_output, torch.argmax(labels.long(), axis=1))
            else:
                loss = args.loss_fn(audio_output, labels)
                # print(f'Nominal loss: /n{loss}')
                # Mask labels if label_mask is not None, do mean reduction on filtered values
                if label_mask is not None:
                    label_mask = label_mask.to(loss.device)
                    if loss.device != label_mask.device:
                        print(f'loss.device={loss.device}, label_mask.device={label_mask.device}')
                    loss = (loss*label_mask).sum() / label_mask.sum() / loss.shape[0]
                    # print(f'Reduced loss: /n{loss}')
        ...
        # print(f'args.loss_fn: {args.loss_fn}')
        # print(f'A_loss:\n{A_loss}')
